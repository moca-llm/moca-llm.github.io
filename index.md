...
The radar plot shows not just the mean AMCE score, but the 95% confidence interval of the AMCE score. Since the AMCE score is the difference in probability change, if the upper/lower bound of the confidence interval crosses 0, it means the tendency is not statistically significant given the current data. The confidence interval is computed through Bootstrapping. If the 95% CI crosses 0, we report the maximal probable tendency towards one attribute (upper/lower bound on the AMCE).